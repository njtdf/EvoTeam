...
## 2026-08-20 23:06:38 — v0.7.7: FTS5 搜索升级 + 总览页面重构
...
## 2026/8/21 10:25:27 - v0.7.8: graduation.js + DB/KB sync + frontend

### Added
- lib/graduation.js (new): 3 NNU standard requirement templates (master 10/phd 8/bachelor 6), weighted progress, risk auto-calc, syncToDb, indexToKb, apply(Cordis)
- scripts/verify-weekly-loop.mjs: e2e weekly report loop verification
- server.js: 6 graduation routes (GET /api/graduation/:id, /all, /categories, /template/:role; POST /seed; PUT /requirement/:reqId)
- teacher.html + teacher.js: graduation tab in student mgmt section (progress bar + stats + requirement list grouped by category, editable status)
- student.html + student.js: read-only graduation view (progress + stats + requirement list, no edit)
- app.css: .graduation-bar-container, .graduation-bar-fill, .risk-low/medium/high/critical, .risk-badge
- Seed data: 18 students all seeded (15 master, 3 bachelor), KB indexed 18 graduation docs

### Audit Results (runtime-verified, not trusting handoff summary)
- Weekly report core loop backend 100% working: /api/report/s01 returns 6 sections + meta + raw, 0 parse_errors [confirmed]
- Handoff misdiagnosed Bug#1: switchToCockpit at teacher.js:66 already calls loadStudents when empty [confirmed]
- Audit script false positives: cockpit/meeting/kanban tagged PLACEHOLDER but have full template code [likely]

### Verification
1. Teacher GET /api/graduation/s01 -> 200, master, 10 reqs, 8 categories [confirmed]
2. GET /api/graduation/all -> 200, 18 students [confirmed]
3. PUT req -> in_progress -> progress 8%, risk high [confirmed]
4. PUT req -> done -> progress 15%, risk high, met_at auto-filled [confirmed]
5. Student login s01/changeme -> GET /api/graduation/s01 -> 200, progress 15% [confirmed]
6. Weekly loop regression -> all 200 [confirmed]
7. KB docs -> 18 graduation/* docs [confirmed]

### Confidence
- [confirmed] graduation.js backend + API routes + seed data + DB/KB sync
- [likely] frontend rendering (JS structure correct, but not browser-verified)
- [needs verification] requirement template content matches actual NNU program (no PDF found)

### Next
- User verify in browser: /teacher -> graduation tab -> select student -> change status -> verify progress bar
- User cross-check: requirement template vs actual NNU training program, adjust weights/items manually
- W6b: lib/decisions.js (decision record module)
- Route A: fix real placeholder pages (todo/notify/projects under 1.5KB)


## 2026-08-21 2026/8/21 12:19:24 — v0.7.9 W6b decisions frontend + W7b kanban state machine + W8 lab-state.js
...
## 2026-08-21 2026/8/21 13:03:57 — v0.7.10 W9/W10/W12/W14/W15 (Roadmap 2.1 剩余波次)
...
## 2026-08-21 07:24:42 — v0.7.11 毕业页下拉框CSS修复 + Roadmap 2.1核对

### 修复内容

- Bug: 毕业页学生下拉框不可见 — 全局 .student-select 设为 color:#fff + 透明背景, 浅色背景上不可见
- 根因: .student-select 基础样式 (app.css:307) 原为深色 header 设计, 毕业页是浅色背景
- 修复: app.css 追加 .graduation-view .student-select 规则, !important 强制白底黑字+灰边框+focus绿色高亮
- 额外修复: 补齐 graduation-bar-container / graduation-bar-fill 类名

### 验证结果 (Playwright computed styles)

- background=rgb(255,255,255) [白底 OK]
- color=rgb(51,51,51) [黑字 OK]
- border=1px solid rgb(217,217,217) [边框 OK]
- 16个学生选项正确加载 (s01-s12 + u01-u03 + 占位符)
- 选择s01后毕业数据正确显示 (master, 15%, 1/10, high risk)

### Roadmap 2.1 完成度核对

| 波次 | 交付物 | 状态 |
|---|---|---|
| W6a | valuecycle.js 状态升级 | 完成 |
| W6b | decisions.js 新建 | 完成 |
| W6c | 填真实毕业状态数据 | 人工(只有导师能做) |
| W7a | trajectory.js 新建 | 完成 |
| W7b | kanban.js 状态机 | 完成 |
| W8 | lab-state.js + 奖励信号 | 完成 |
| W9 | transition() 升级 | 完成 |
| W10 | 外部感知接入 | 完成(MCP部分defer) |
| W11 | AG-UI 协议引入 | Defer(需前端重构) |
| W12 | skill-extractor.js | 完成 |
| W13 | A2A 协议引入 | Defer(需cordis runtime) |
| W14 | LabOS Cockpit | 完成 |
| W15+ | Value Cycle Dashboard | 完成 |

总结: 10/13 波次完成. W6c=人工任务. W11/W13=defer. 可做的全做完了.

[确定] Playwright computed styles 验证通过


## 2026-08-21 12:22:56 — Roadmap 2.2 创建

- **内容**: 基于李开复四块拼图（大脑/地图/导航/操作系统）框架，诊断 LabOS 当前 6 个功能的数据孤岛问题
- **核心交付**: 8 阶段计划，~10.5 天；P0=本体(ontology.js)+飞轮(flywheel.js)；P1=周报/任务/组会精打磨；P2=项目重做/知识库回流；P3=SOTA文献动态
- **关键洞察**: 6 个功能当前是 6 个竖井，缺少本体（地图）和飞轮（传送带），数据不在功能间流动，AI 无法跨功能推理
- **输出文件**: D:/OneDrive/7-SideWork/AutoProf/AutoProf-Architecture-Roadmap2.2.md
- **置信度**: [确定] 基于 Roadmap 2.1 完成度审计 + 李开复框架映射


## 2026-08-21 12:46:23 — Roadmap 2.2 Phase 1: 组织地图 (Ontology 2.0)

### 新增文件
- **lib/ontology.js** (295行): 组织地图——8类实体 + 9种关系，从现有数据源自动构建
  - 实体: student(19) / project(13) / task(40) / weekly_report(13) / summary(12) / meeting(4) / submission(0) / paper(20)
  - 关系: belongs_to / authored / summarized_by / assigned / derived_from / mentions / authors / part_of / relevant_to
  - 自动推导 147 条关系，叠加手动关系存储到 labos/ontology.json
  - getEntityGraph(student,s01) 一次查询返回 35 个关联实体 (1项目+2周报+1总结+27任务+4会议)
  - 自然语言 CRUD: 宋禧 属于 AI Datacenter -> 自动解析并添加关系
  - apply(ctx, config) Cordis 形态保留

### 修改文件
- **server.js**: 新增 7 条 ontology API 路由
  - GET /api/ontology/stats (teacher)
  - GET /api/ontology/entities (auth)
  - GET /api/ontology/graph/:type/:id (auth)
  - GET /api/ontology/search?q= (auth)
  - POST /api/ontology/relation (teacher)
  - DELETE /api/ontology/relation/:id (teacher)
  - POST /api/ontology/nl (teacher)
- **lib/ai-context.js**: buildStudentContext 末尾注入本体图
  - AI 与学生对话时自动看到: 项目(N) + 周报(N) + 总结(risks:N) + 任务(N) + 会议(N)
  - AI 不再需要翻 7 个文件理解学生全貌

### 验证
- [确定] ontology.js import 成功，getOntologyStats() 返回正确统计
- [确定] getEntityGraph(student,s01) 返回 35 个关联实体
- [确定] 自然语言 CRUD 测试通过
- [确定] 7 条 API 路由全部 HTTP 200
- [确定] ai-context.js 注入本体图后不崩溃
- [确定] 服务器重启正常

### 关键设计决策
- **本体不复制数据**: 是索引层，从 students.yaml / tasks.json / reports/ / summaries/ / meetings/ / submissions.json / external-events.json 实时读取
- **手动关系叠加**: 自动关系每次实时构建，手动关系存 labos/ontology.json
- **自然语言不调 AI**: 纯模式匹配，避免幻觉

### 版本: 0.7.11 -> 0.7.12


## 2026-08-21 12:58:21 — Roadmap 2.2 Phase 2: 飞轮回流引擎 (Flywheel Engine)

### 新增文件
- **lib/flywheel.js** (310行): 飞轮回流引擎——事件驱动数据回流管道
  - 4 条管道: report_submitted / meeting_ended / task_done / paper_indexed
  - 每条管道 2-4 个步骤，单步失败不中断后续
  - trigger(event, payload) 异步执行，fire-and-forget，不阻塞 HTTP
  - buildFlywheelContext(studentId) 聚合: 最近总结/任务状态/会议行动项/轨迹/关联图/飞轮事件
  - 日志: labos/flywheel-log.json (ring buffer, 最近 200 条)
  - apply(ctx, config) Cordis 形态保留

### 修改文件
- **server.js**: 3 处事件源挂载飞轮 trigger
  - POST /api/submit: 周报提交后 trigger report_submitted (index KB + log trajectory)
  - POST /api/meeting/upload: 会议抽取后 trigger meeting_ended (index KB + log trajectory per student)
  - POST /api/tasks/:id/transition: 任务转 done 时 trigger task_done (log milestone)
- **server.js**: 新增 2 条 flywheel API 路由
  - GET /api/flywheel/stats (teacher) — 管道统计
  - GET /api/flywheel/log (teacher) — 事件日志
- **lib/ai-context.js**: buildStudentContext 末尾注入飞轮上下文
  - AI 与学生对话时自动看到: 最近AI总结 + 任务状态 + 会议行动项 + 轨迹 + 关联图
  - AI 不再只看静态数据，能看到最近发生了什么 (导航系统)

### 验证
- [确定] flywheel.js import 成功，getFlywheelStats() 返回正确管道列表
- [确定] buildFlywheelContext(s01) 返回 6 段上下文 (总结/任务/会议/轨迹/关联/事件)
- [确定] 端到端: POST /api/tasks/T-002/transition done → 飞轮自动触发 → 2 步全 ok → 日志记录
- [确定] GET /api/flywheel/stats 返回 1 event (task_done, status=ok)
- [确定] GET /api/flywheel/log 返回事件详情 + 步骤状态
- [确定] ai-context.js 注入飞轮上下文后不崩溃
- [确定] 服务器重启正常

### 李开复四块拼图映射
- 大脑(模型): DeepSeek — 已有
- 地图(本体): ontology.js — Phase 1 完成
- 导航(实时上下文): flywheel.js buildFlywheelContext — Phase 2 完成
- 操作系统(调度+执行): trigger + pipeline — Phase 2 完成

### 版本: 0.7.12 -> 0.7.13

## 2026-08-21 13:47:43 [CST] — Phase 3: 周报精打磨 (飞轮双向回流)

### [确定] 后端: GET /api/report-context/:id 新增
- 返回结构化上下文: last_summary (AI总结 risks/suggestions) + meeting_actions (分配给该学生的会议行动项) + open_tasks (未完成任务) + flywheel_recent (最近3条飞轮事件) + ontology_links (本体关联统计)
- 所有数据从现有 lib/ 函数聚合,零新文件存储
- 权限: requireAuth (学生看自己,导师看任意)

### [确定] 后端: POST /api/submit 返回 flywheel 确认
- response 增加 flywheel: true 字段
- 学生提交后前端可显示"已索引知识库+记录轨迹"

### [确定] 前端学生端: 上次反馈面板
- student.js: 新增 reportContext ref + loadReportContext() + toggleFeedback()
- onMounted 调用 loadReportContext(u.id) 预加载
- doSubmit 成功后 showToast 包含飞轮确认 + 重新加载 context
- student.html: 编辑器上方新增"上次反馈"按钮 + 折叠面板
  - 展示: 上次风险标签 + 上次建议 + 会议行动项 + 未完成任务 + 飞轮最近事件
- CSS: .report-feedback-card (淡蓝背景) + .rf-section/.rf-label/.rf-task/.rf-flywheel

### [确定] 前端导师端: cockpit 飞轮上下文
- teacher.js: 新增 reportContext ref,onStudentChange 中 Promise.allSettled 后额外加载 /api/report-context/:id
- teacher.html: cockpit 右侧 pane 在周报卡片之后新增"飞轮上下文"卡片
  - 展示: 未完成任务(最多8条) + 会议行动项(最多5条) + 飞轮最近事件 + 本体关联计数
- 修复: v-if/v-else-if 链断裂 — 飞轮卡片放在 v-else 块之后,用独立 v-if,不干扰 report 的条件链

### [确定] 数据流
- 学生写周报 → 看到上次反馈(风险/建议/任务/会议行动) → 提交 → 飞轮触发(index KB + log trajectory) → 学生看到飞轮确认
- 导师选学生 → 看到周报 + AI总结 + 飞轮上下文(任务/会议/事件) → 与AI聊天时上下文已注入飞轮数据
- 双向: 周报→飞轮→下次周报反馈 (数据飞轮闭环)

### 版本: v0.7.13 → v0.7.14

## 2026-08-21 14:44:30 (Asia/Shanghai) — Phase 4: 李开复执行闭环引擎 (v0.7.15)

### [确定] 新建 lib/promise-ledger.js — 承诺账本核心模块
- 数据模型: promises.json (promise_id, promiser_name, promiser_student_id, promise, deadline, promisee, status, fulfilled_at)
- 4 大组件全部实现:
  1. 承诺账本 CRUD: createPromise / getPromises / fulfillPromise / markOverdue
  2. 目标树: getGoalTree() — 从 students.yaml project + tasks.json 聚合, 计算项目进度
  3. 逾期预警: getUpcomingDeadlines(days=7) + getPromiseUrgency() 分级 (urgent/warning/overdue/done)
  4. 言行一致指数: getConsistencyIndex(studentId) — 180天滚动窗口, fulfilled/total, A/B/C/D 评级
- 姓名→ID 权威匹配 (AI 只出 name, 服务端 matchNameToId, 防幻觉 ID)
- autoExtractFromMeeting(date): 读纪要 → AI 抽取 → matchName → 创建承诺
- Cordis apply() 形态保留

### [确定] lib/ai.js 扩展 — extractPromises()
- 复用 extractActions 的 response_format:{type:'json_object'} 模式
- temp 0.1 (比 extractActions 的 0.2 更低, 承诺抽取需精确)
- System Prompt: 抽取 {promises:[{promiser, promise, deadline, promisee, source_section}]}
- 明确要求 promiser 必须是 roster 内姓名精确匹配, 否则空字符串
- 降级: 无 API key → 返回 {promises:[], status:'no_api_key'}

### [确定] server.js — 8 条承诺路由
| Method | Path | Auth | 功能 |
|---|---|---|---|
| GET | /api/promises/stats | teacher | 统计 |
| GET | /api/promises/overdue | teacher | 逾期列表 |
| GET | /api/promises/upcoming | teacher | 即将到期 (query: days) |
| GET | /api/promises/consistency | teacher | 全部学生一致性指数 |
| GET | /api/promises/consistency/:id | any | 单个学生 |
| GET | /api/promises/goal-tree | any | 目标树 |
| GET | /api/promises | any | 承诺列表 (query: student_id/date/status) |
| POST | /api/promises | teacher | 手动创建承诺 |
| POST | /api/promises/extract/:date | teacher | 从会议自动抽取 |
| PUT | /api/promises/:id/fulfill | teacher | 标记兑现 |

### [确定] lib/flywheel.js — meeting_ended 管道加承诺抽取步骤
- 新增步骤: extract_promises_from_meeting (在 log_trajectory 之后, build_context 之前)
- 上传会议纪要 → 飞轮自动抽取承诺 (无需手动触发)
- try/catch 包裹, 承诺抽取失败不影响其他飞轮步骤

### [确定] 前端 — teacher.js + teacher.html + app.css
- teacher.js: 新增 refs (promiseStats, consistencyData, overduePromises, upcomingPromises, showPromisePanel)
  + loadPromiseData() 函数 (并行加载 4 个 API)
  + fulfillPromiseAction(id) 函数
  + switchToDashboard 中调用 loadPromiseData()
  + return block 新增所有 ref/函数
- teacher.html:
  - 逾期预警栏 (overdue-warning-bar): 有逾期承诺时显示红色横幅, 点击展开承诺账本
  - 承诺账本面板 (promise-list): 展示逾期 + 即将到期承诺, 可标记兑现
  - 言行一致指数网格 (consistency-grid): 每个学生卡片显示百分比 + 评级 + 进度条
  - 点击学生卡片跳转周报
- app.css:
  - Phase 4 task detail modal 样式 (.task-detail-modal, .td-section, .transition-row)
  - 增强任务卡片样式 (.task-card-enhanced, .task-card-desc, .tag-project, .tag-ref)
  - 承诺账本样式 (.promise-item, .promise-badge, .urgency-*)
  - 一致性指数样式 (.consistency-card, .consistency-ring, .consistency-bar-*)
  - 逾期预警样式 (.overdue-warning-bar, .ow-*)
  - 目标树样式 (.goal-tree-*)

### [确定] 端到端验证结果
- API 全部返回 200
- AI 抽取: 2026-08-18 会议 → 3 条承诺 (宋禧/常申奥/陈光), 全部姓名匹配 (unmatched=0)
- 一致性指数: 宋禧 50%(C), 常申奥 0%(D), 陈光 0%(D)
- 逾期预警: P-002 宋禧 (deadline 08-25) 出现在 upcoming 列表
- 目标树: 10 项目聚合, 总进度 3%
- 飞轮: meeting_ended 管道含 extract_promises_from_meeting 步骤

### 版本
- v0.7.14 → v0.7.15 (Phase 4 新功能, 第三位)
- VERSION + api.js APP_VERSION + HTML cache-bust v=0715 全部同步

## 2026-08-21 23:33:13 [v0.7.16] student profile + progress bars + dropdown fix

1. Student dropdown fix: CSS white-on-white to var(--card)+var(--text)
2. Removed consistency index from dashboard, moved to student profile page
3. Student profile page (teacher): lib/student-profile.js + 3 API routes + card grid + detail modal
4. Task progress bars: kanban card mini progress bar (todo=0/in_progress=50/done=100/blocked=30)
5. Student-side profile: graduation progress + task progress + capability(7) + consistency + KB stats
6. Undergrad accounts: u01/u02/u03 (password: changeme)

API verification: all 200 (teacher/student/undergrad)
Version: 0.7.15 to 0.7.16

## 2026-08-21 23:47:06 [v0.7.17] course page MVP - Power System Fundamentals

1. Course data: labos/courses/电力系统基础.json (9 chapters, 17 weeks, 4 objectives, 6 assessment items)
2. lib/course.js: loadCourse/listCourses/getCourseDetail/updateCurrentWeek/getCourseProgress
3. server.js: 4 routes - GET /api/courses, GET /api/course/:name, PUT /api/course/:name/week (teacher), GET /api/course/:name/progress
4. teacher.html+js: Course tab in Teaching section - 5 subtabs (schedule/objectives/chapters/assessment/resources)
5. student.html+js: Course tab for undergrads - same view, no week-edit
6. CSS: course-header, schedule-week, chapter-grid, assessment-table, resource-cards
7. Progress bar: overall course progress (week/total * 100%), week-level current/past/future colors

Source: 2 docx files (syllabus + OBE plan) parsed via mammoth
API test: all 200 (list/detail/progress/update-week/undergrad-access)
Version: 0.7.16 -> 0.7.17

## 2026-08-22 08:13:05 — v0.7.18 Bug Fix: 3 Critical UI Crashes
...
## [2026-08-22 00:29:11] v0.7.19 -- fix: student kanban white screen

**confidence: [confirmed]**

### root cause
student.html kanban section (line 206) had misplaced {{ gradSummary.template_note }}
gradSummary = ref(null) in student.js:623, onMounted does not load it
kanban template renders unconditionally -> null.template_note -> TypeError -> Vue crash -> white screen

### fix
- removed misplaced gradSummary.template_note line from kanban section
- scanned all ref(null) vars for unguarded property access in HTML -> all safe
- teacher.html gradSummary has v-if guard, safe

### verified
- s01/changeme login 200, /api/tasks 27 tasks
- u01/changeme login 200
- t01/lab123 login 200
- student.html: kanban-view exists, gradSummary refs=0
...
## 2026-08-22 Wave 7: 价值目标树 + 每日信息 + 灵感工厂

### 后端 (lib/ 新建)
- `lib/goal-tree.js`: buildValueGoalTree() — 树干=课题组研究方向, 分支=学生课题, 叶子=任务. 8方向/18学生/40任务
- `lib/ideas.js`: 灵感工厂 — storeIdea/listIdeas/sparkIdeasStream(SSE). 读取每日信息+目标树 → DeepSeek 流式生成
- `server.js`: 7条新路由 — /api/goal-tree, /api/goal-tree/dependencies, /api/daily-brief, /api/ideas (GET/POST/DELETE/PUT-status), /api/ideas/spark (SSE)

### 前端教师端
- 价值链 Tab: 价值目标树展示 (可折叠分支 + 学生进度条 + 依赖chips)
- 核心段: 每日信息 Tab (逾期/今日到期/进行中/外部动态/目标树概览)
- Agent段: 灵感工厂 Tab (SSE流式AI输出 + 创意库列表)
- 修复: 进度条 NaN bug → 替换为 gt-progress-fill div

### 前端学生端
- 核心段: 每日信息 Tab (只显示自己的任务)
- Agent段: 灵感工厂 Tab (只显示自己的创意)
- hash 路由: +daily +ideas

### CSS
- .gt-* (价值目标树: branch/student/progress-bar/deps)
- .db-* (每日信息: stat-pill/task-item/event-item/tree-summary)
- .ideas-* (灵感工厂: textarea/stream/idea-card)

### API 测试结果
- GET /api/goal-tree: 200 — 8 directions, 18 students, 40 tasks
- GET /api/goal-tree/dependencies: 200 — 18 nodes, 75 links
- GET /api/daily-brief (teacher): 200 — date 2026-08-22, 0 overdue, 3 in_progress
- GET /api/daily-brief (student s01): 200 — 0 overdue, 1 in_progress (学生只看自己的)
- GET /api/ideas (teacher): 200 — 2 ideas
- GET /api/ideas (student s01): 200 — 0 ideas (学生只看自己的)

### 语法/结构验证
- teacher.js: node --check PASS
- student.js: node --check PASS (修复 await in non-async hash handler)
- teacher.html: 638 divs BALANCED
- student.html: 234 divs BALANCED

### 版本
- 0.7.20 → 0.7.21 (第三位, 新功能增量)
## 2026-08-22 v0.7.22 Task board refinement + Ideas sharing MVP

### Task board refinement
- Student kanban: added stats bar (total/todo/in_progress/done/overdue + progress pct)
- Teacher create task: default deadline = today+7 days (openTaskModal)
- Task deadline -> calendar: already wired in lib/calendar.js (loadCalendarEvents pulls tasks.json)
- Overdue card highlight CSS (.is-overdue)

### Ideas brainstorming/sharing MVP
- lib/ideas.js: added shared + liked_by fields, listIdeas shared filter, likeIdea() toggle
- server.js: GET /api/ideas?shared=true returns all shared ideas; POST /api/ideas/:id/like toggle
- Student + Teacher: shared ideas wall (creative wall) with like buttons
NaN
- Cross-user: s02 can see s01 shared ideas, vice versa

### API tests
- POST /api/ideas (shared=true): 200, id=2026-08-22-003
- GET /api/ideas?shared=true: 200, count=1
- POST /api/ideas/:id/like: 200, toggle liked_by []
- s02 sees s01 shared idea: true
## 2026-08-22 v0.7.23 RSS从任务页移到灵感工厂 + 灵感工厂去掉每日信息

### 用户要求
- 每日研究动态(RSS)不要放到任务看板里，移到灵感工厂
- 灵感工厂去掉"每日信息"文案
- 确认周报和任务精细化打磨状态

### 改动
- teacher.html: 看板欢迎行移除RSS区块(welcome-news)，加注释"RSS moved to 灵感工厂"
- teacher.html: 灵感工厂页面新增"每日研究动态(RSS)"卡片(rss-list + rss-item)
- teacher.html: 灵感工厂intro从"每日信息+价值目标树"改为"研究动态+价值目标树"
- teacher.js: switchToKanban移除loadNews()，switchToIdeas增加loadNews()
- student.html: 灵感工厂intro从"每日信息+价值目标树"改为"研究动态"（学生端无价值树）
- student.html: 灵感工厂新增RSS卡片
- student.js: 新增news ref + loadNews()函数 + switchToIdeas调用 + return block

### 周报打磨状态(v0.7.20完成)
- 学生端: 报告列表(日期+状态) ✓ / 双栏编辑器(MD左+预览右) ✓ / 模板自动加载 ✓ / AI辅助草稿 ✓ / 提交toast ✓ / 历史查看 ✓
- 导师端: 学生卡片网格 ✓ / 下拉选择 ✓ / 简报生成 ✓ / 聊天SSE ✓
- [待验证] 用户尚未在浏览器端逐一确认

### 任务打磨状态(v0.7.22完成)
- 学生端: 统计条(总/todo/in_progress/done/overdue+%) ✓ / 任务卡片(截止日期+逾期高亮) ✓ / 状态切换 ✓
- 导师端: 看板统计 ✓ / 创建任务(默认截止today+7) ✓ / 任务详情modal ✓ / 逾期高亮 ✓
- [待验证] 用户尚未在浏览器端逐一确认

### 语法/结构验证
- teacher.js: node --check PASS
- student.js: node --check PASS
- teacher.html: 644/644 divs BALANCED
- student.html: 246/246 divs BALANCED

### 版本
- 0.7.22 -> 0.7.23 (第四位, 小修)

## 2026-08-22 v0.7.24

### Changes
1. Removed Daily Brief tab from teacher + student
2. Todo filters by current user only (teacher sees teacher tasks)
3. Teacher weekly report: stat bar + student cards, removed brief button
4. Global AI chat bar at bottom of all pages (50vh expand on click)
5. Ideas factory: kept RSS news, removed daily brief

### Bug fixes
- streamChat (api.js): handle both msg.chunk and msg.content keys
- streamChat (api.js): correctly handle [DONE] marker to trigger onDone
- global-chat SSE (server.js): callback mode instead of for-await
- APP_VERSION syntax: const uses = not :

### API test results
| Endpoint | Status | Result |
|---|---|---|
| POST /api/login (teacher) | 200 | OK |
| GET /api/dashboard | 200 | 15 students, 12 reported, 3 missing |
| GET /api/tasks | 200 | 40 total, 0 teacher own (correct filter) |
| POST /api/global-chat (SSE) | 200 | 29 chunks, AI reply correct |

## 2026-08-22 v0.7.25 — Agent team moved out of dashboard + AI AGENT rename + mouseleave

### Changes
1. Removed Agent grid + Agent chat panel from dashboard (kn-scroll-area)
2. Added Agent chat panel to agents tab (Agent 实时状态)
3. Renamed "AI 助手" to "AI AGENT" in teacher + student + meeting-live
4. Global chat bar: added @mouseleave to auto-collapse when mouse leaves expanded panel

### Verification
- teacher.html div balance: 628/628 OK
- student.html div balance: 240/240 OK
- index.html div balance: 23/23 OK
- Version sync: VERSION + api.js + 4 HTML files all 0.7.25
- teacher agent grid in dashboard: false (removed)
- mouseleave handler: true (both teacher + student)

## 2026-08-22 v0.7.26 C-Level Agents + Context-Aware Global Chat

### What Changed

**server.js:**
  * Added AGENT_PROMPTS map with 7 C-level agents: CEO CFO CTO CMO CAIO CBO CHO
  * /api/global-chat now reads agent_id and selects matching system prompt
  * Fixed teacher branch overriding agent_id with hardcoded manager prompt
  * Fixed SyntaxError from AGENT_PROMPTS inserted mid-statement

**teacher.js:**
  * Added cAgents array (7 agents with emoji icons and gradients)
  * Added tabAgentMap (tab to default agent mapping)
  * Added globalChatAgentId ref and globalChatAgent computed
  * Added autoSwitchAgent(tab) called by watch(activeTab)
  * sendGlobalChat now sends agent_id in request body
  * All new refs added to setup return block

**teacher.html:**
  * Global chat bar: static text replaced with select dropdown
  * click.stop on select prevents expand/collapse interference

**student.js:** sendGlobalChat sends agent_id=caio

**student.html:** label updated to CAIO AI Agent

**app.css:** added global-chat-agent-select styles

### Testing
  * Version check 0.7.26 matches (no infinite reload)
  * Login t01/lab123 works
  * Global chat CEO: SSE streaming CEO prompt used
  * Global chat CTO: SSE streaming CTO prompt used context injected
  * teacher.html dropdown with cAgents present agent-grid in Agent tab intact
  * div balance 628/628 OK

## v0.7.27 — 2026-08-22 23:50 — Agent 实时状态页换为 7 位 C-Level 智能体

### 背景
用户要求"Agent 实时状态"页面里的旧任务级 Agent（manager/summary/meeting/stt/skill/progress/review/interview/valuecycle）替换为 7 位新的 C-Level 智能体（CEO/CFO/CTO/CMO/CAIO/CBO/CHO）。上个模型做了补丁但卡在 DIV 不平衡（636/637）。

### 修复内容
1. **DIV 平衡修复**: teacher.html 第 1180 行有坏行 `</div> — 点击"创建 Agent"添加</div>`（之前 bad find-replace 残渣），改为正确的 `</div>` → 636/636 平衡 ✅
2. **验证 teacher.js**: `cAgents` const 数组含 7 个 agent（每个有 status:'active' + capabilities 数组），`selectCLevelAgent()` 函数 + `autoSwitchAgent()` 标签映射 + 全部在 return 块中 ✅
3. **验证 server.js**: `AGENT_PROMPTS` 含 7 个 C-Level system prompts + manager fallback；`/api/global-chat` 路由正确读取 `req.body.agent_id` 选择对应 prompt ✅
4. **teacher.html Agent 网格**: `v-for="a in cAgents"` 显示 7 个 C-Level 卡片 + 下方"任务级 Agent"区（旧 9 个，opacity 0.7）✅
5. **版本号**: VERSION → 0.7.27，api.js APP_VERSION → 0.7.27，所有 HTML ?v=07.27 ✅
6. **清理**: 删除 6 个临时 .mjs 文件 ✅

### 端到端测试
| 测试项 | 结果 |
|---|---|
| Login (t01/lab123) | 200 + cookie OK |
| GET /teacher | 200, 103KB |
| 页面含 cAgents + selectCLevelAgent | true |
| /api/version | 0.7.27 匹配 |
| /api/global-chat (agent_id=ceo) SSE | 200 + 流式输出 "你好。我是课题组..." |

### 下一步
用户可在浏览器验证：t01 登录 → Agent 标签页 → 应看到 7 个 C-Level 智能体卡片 → 点击"💬 对话" → 底部全局聊天栏展开 + 自动选中该智能体。

## 2026-08-22 v0.7.28 — 删旧 Agent + 名称顺序 + CSS 修复 + 文件夹精简

### 用户要求 4 件事

1. 旧任务级 Agent 全部删除
2. C-Level 名称英文放前面 (CEO 首席执行官)
3. AGENT 实时状态 sub-tab 绿字绿底看不见
4. cordis-main 文件夹精简，不用的删掉

### 执行结果

**1. 旧任务级 Agent 全部删除 ✅**
- teacher.js: agents = ref([]) 清空 (仅保留用于 createAgent 自定义)
- 删除 sendAgentChat / activeAgentId / agentChatInput / agentStreaming / agentStreamText / activeAgent 全部代码
- teacher.html: 删除任务级 Agent section + 旧 Agent Chat Panel + C-Level 卡片上的 editAgent/deleteAgent 按钮
- 总览页 v-if 条件移除 (改为始终显示)

**2. C-Level 名称英文在前 ✅**
- teacher.html 卡片: {{ a.shortName }} {{ a.name }} → "CEO 首席执行官"
- 下拉菜单: {{ a.icon }} {{ a.shortName }} {{ a.name }}
- 7 个 C-Level: CEO/CFO/CTO/CMO/CAIO/CBO/CHO

**3. sub-tab CSS 修复 ✅**
- 根因: 两套 .sub-tab.active 定义冲突 — #07C160 绿底 + #07C160 绿字 = 不可见
- 修复: 删除重复定义，改为浅绿底 #e8f5e9 + 深绿字 #07C160
- 影响: Agents/Skills/Plugins/MCP 四个 sub-tab 全部修复

**4. cordis-main 文件夹精简 ✅**
- 删除 9 个未用 packages: core, create, group, hmr, include, loader, logger-console, timer, utils
- 保留 packages/lab-brief (唯一被 server.js + lib/summary.js 引用)
- 删除嵌套空目录 cordis-main/cordis-main
- 删除 TS 配置: tsconfig.base.json, tsconfig.json, tsconfig.test.json, vitest.config.ts, yakumo.yml
- 删除 err.txt + 12 个临时 _*.mjs 脚本
- packages/ 现在只含 lab-brief

### 端到端验证

- Login t01/lab123: 200 ✅
- /api/version: 0.7.28 ✅
- /teacher 页面: 200, 100KB ✅
- shortName 在 name 前: true ✅
- sendAgentChat 已删: true ✅
- cAgents 存在: true ✅
- Global chat (ceo) SSE: 200 ✅
- Custom agents API: 200 {"agents":[]} ✅ (空，旧 agent 已删)
- 7 个 C-Level 确认: CEO/CFO/CTO/CMO/CAIO/CBO/CHO ✅
- DIV 平衡: 620/620 ✅
- 服务器重启: 0.7.27 → 0.7.28 ✅

### 版本同步
- VERSION: 0.7.28
- api.js APP_VERSION: 0.7.28
- 所有 HTML ?v=07.28

## 2026-08-22 v0.7.29 — 修复全局聊天框打字时自动收起

### 问题
每个页面底部 AI AGENT 聊天栏，用户一打字就最小化，无法输入。

### 根因
- teacher.html / student.html 的 .global-chat-bar 有 @mouseleave="globalChatExpanded = false"
- 鼠标稍微移出聊天栏边界（打字时手部自然移动）就触发 mouseleave → 收起
- 即使 input 有焦点也不阻止收起

### 修复
- 新增 onChatMouseLeave() 函数：检查 document.activeElement 是否为 .global-chat-input
- 如果用户正在输入框中打字（焦点在输入框），不收起
- 只有鼠标离开且输入框无焦点时才收起
- 同时修复 teacher.html + student.html + teacher.js + student.js（4 文件）

### 验证
- Teacher 页面 200, onChatMouseLeave 存在 ✅
- Student 页面 200, onChatMouseLeave 存在 ✅
- Version 0.7.29 ✅
- DIV 平衡: teacher 620/620, student 240/240 ✅

## 2026-08-22 Wave 8b: Meeting->Task Flywheel + Task Refinement + Report Risk->Task (v0.7.30)

### Backend (server.js)
1. GET /api/tasks - Added is_overdue field (deadline < today and status != done)
2. POST /api/meeting/upload - Chained autoExtractFromMeeting for promise ledger auto-population
3. POST /api/tasks/from-meeting - Added already_promoted check (source_ref match) + default deadline
4. POST /api/tasks/from-meeting/bulk (NEW) - Batch promote all matched meeting actions
5. POST /api/tasks/from-risk (NEW) - Create task from weekly report AI risk (source=weekly_risk, priority=high)
6. POST /api/tasks/batch-deadline (NEW) - Set default deadline (today+7) for tasks without one. 27 updated
7. GET /api/report-context/:id - Added done_tasks (last 5 completed) alongside open_tasks

### Frontend (teacher.html + teacher.js)
1. Meeting: Promote All to Tasks bulk button + Promoted badge on already-promoted items
2. Kanban: View toggle By Status / By Project + Set Deadlines button + overdue card red border
3. Report: Each AI risk gets Track button -> creates tracking task via from-risk endpoint
4. Unmatched meeting actions get yellow background (CSS)

### Frontend (student.html)
1. Feedback panel shows done_tasks (last 5 completed) alongside open_tasks

### Verification Results
| Test | Result |
|------|--------|
| GET /api/tasks is_overdue | PASS (41 tasks) |
| POST /api/tasks/batch-deadline | PASS (27 updated) |
| POST /api/tasks/from-risk | PASS (T-041 created) |
| POST /api/tasks/from-meeting/bulk | PASS (3 already promoted) |
| POST /api/tasks/from-meeting already_promoted | PASS (true for T-013) |
| GET /api/report-context done_tasks | PASS |
| GET /api/calendar/events task deadlines | PASS (40 events) |
| GET /api/promises/stats | PASS (4 promises) |
| teacher.js syntax | PASS |
| teacher.html div balance | PASS (630/630) |
| student.html div balance | PASS (243/243) |
| Server v0.7.30 running | PASS |

### Data Flywheel
Weekly report risk -> Track button -> Kanban task (weekly_risk)
Meeting minutes -> AI extract -> Promote All -> Kanban task + Promise ledger
Task completion -> flywheel trigger -> next report Done Tasks feedback
Task deadline -> Calendar event

## 2026-08-22 — v0.7.30 — Sidebar Collapse/Expand Toggle

### What
  * ◀ collapse button in sidebar header (teacher + student)
  * ☰ floating restore button when sidebar is collapsed
  * Smooth width transition CSS (0.25s ease)
  * Fixed student.js line 843 syntax error (const globalChatAgentId: -> = ref())
  * sidebarCollapsed ref + toggleSidebar() in both Vue apps + return blocks
  * Cache-bust bumped to v07.30 across all HTML files

### Files
  * public/teacher.html — buttons + class binding
  * public/student.html — same
  * public/js/teacher.js — sidebarCollapsed + toggleSidebar
  * public/js/student.js — same + syntax fix
  * public/css/app.css — collapsed / toggle / collapse-btn + transition

### Test
  * teacher login 200, dashboard 200, 15 students
  * student.js + teacher.js syntax check pass
  * git push a1fdff8..7631595

## 2026-08-23 — v0.7.30 — Repository rename: cordis-main to EvoTeam

### What
  * Renamed GitHub repo from AutoProf-LabOS to EvoTeam
  * Renamed local folder from cordis-main to EvoTeam
  * Updated all 24 hardcoded path references (scripts, summaries, AGENTS.md, package.json)
  * Updated package.json name from @root/cordis to evoteam
  * Updated git remote to https://github.com/njtdf/EvoTeam.git
  * Confirmed node_modules NOT tracked by git (already in .gitignore)
  * Server starts successfully from EvoTeam folder, API responds correctly

### Files Changed
  * AGENTS.md - path references
  * package.json - name field
  * scripts/*.mjs (9 files) - absolute path constants
  * labos/summaries/*.json (12 files) - report_file paths

### Notes
  * Old cordis-main folder still exists (locked by Codex app CWD) - can be deleted manually after restart
  * packages/lab-brief/lib/ restored from node_modules/@labos/plugin-lab-brief/lib/
  * New GitHub repo: https://github.com/njtdf/EvoTeam

### 2026-08-23 v0.7.31 — Roadmap 2.3 written

**Done:**
- Wrote roadmap2.3.md: current weekly report architecture analysis (route timing diagram + tech stack), 6 core problems diagnosed
- Agent decision: Waku single-agent loop (~60 lines JS), not multi-agent. Compared Waku/Pi/QM/Multica, chose Waku for thinnest abstraction
- 5-phase plan: Phase 1 weekly report polish, Phase 2 agent-loop.js, Phase 3 tasks, Phase 4 meetings, Phase 5 undergraduate classroom
- CHANGELOG updated, pushed to EvoTeam repo

**Key findings:**
- Weekly report has 9 routes (report/reports/submit/summary/report-context/chat x2/progress-draft x2)
- AI draft (progress.js) depends on Codex rollout summaries — only 1-2 students have history, rest get empty template
- report-context endpoint already provides tasks+meetings+summary+flywheel — but draft generation doesn't use it
- Agent layer is empty shell: agentPrompts dict + buildManagerContext(), no tool-calling loop
- lib/agent-loop.js does NOT exist yet (Phase 2 creates it)

**Next:** Phase 1 weekly report end-to-end polish
