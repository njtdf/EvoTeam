...
import { autoExtractFromMeeting } from './promise-ledger.js'
...
const PIPELINES = {
  ...
  meeting_ended: [
    ...
    {
      name: 'extract_promises_from_meeting',
      fn: async (p) => {
        if (!p.date) return { skipped: true }
        try {
          const result = await autoExtractFromMeeting(p.date)
          return { promises_extracted: result.extracted || 0, unmatched: result.unmatched || 0 }
        } catch (e) {
          console.error('[flywheel] extract_promises failed:', e.message)
