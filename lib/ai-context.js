...
import { buildFlywheelContext } from './flywheel.js'
...
export function buildStudentContext(studentId) {
 ...
 if (vc.advisor_assessment?.notes) {
  ...
  // --- Flywheel context (Roadmap 2.2: AI has the "navigation") ---
  try {
    const fwCtx = buildFlywheelContext(studentId)
    if (fwCtx) {
      parts.push('')
      parts.push(fwCtx)
    }
  } catch (e) {
    // flywheel not critical, fail silently
  }
