...
export function storeIdea({ author_id, author_name, author_role, title, content, tags, source, shared }) {
  ensureDir();
  const now = new Date();
...
export function listIdeas({ author_id, limit, shared } = {}) {
  ensureDir();
  let files;
  ...
  if (shared) ideas = ideas.filter(i => i.shared === true);
...
export function likeIdea(id, userId) {
  const path = join(STORAGE, id + '.json');
  if (!existsSync(path)) return null;
  try {
    const idea = JSON.parse(readFileSync(path, 'utf8'));
    idea.liked_by = idea.liked_by || [];
    if (idea.liked_by.includes(userId)) {
      idea.liked_by = idea.liked_by.filter(u => u !== userId);
    } else {
...
export function apply(ctx, config) {
  ctx.ideas = { store: storeIdea, list: listIdeas, spark: sparkIdeasStream, context: buildSparkContext, status: updateIdeaStatus, delete: deleteIdea, like: likeIdea };
}
